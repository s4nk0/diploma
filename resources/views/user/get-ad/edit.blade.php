<x-app-layout>
    <h2 class="mb-4">Редактировать запись {{__('ad.get_ad')}}</h2>
    <form action="{{route('user.get_ad.update',['get_ad'=>$get_ad])}}" method="post">
        @csrf
        @method('PUT')
        <div class="row mb-3">
            ...
            <div class="col-sm-2">
                <input type="number" min="1" class="form-control" id="rooms_count" name="rooms_count" value="{{$get_ad->rooms_count}}" required>
            </div>
        </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-2 d-flex align-items-center">
                <input type="number" min="1" class="form-control me-2" id="price" name="roommate_count" value="{{$get_ad->roommate_count}}" required>
            </div>
        </div>
                ...
                <div class="row">
                    <div class="col-md mb-2">
                        <div class="form-floating">
                            <input type="number" min="1" name="price_from" class="form-control" id="price_from" value="{{$get_ad->price_from}}" required>
                    ...
                    <div class="col-md mb-2">
                        <div class="form-floating">
                            <input type="number" min="1" class="form-control" name="price" id="price" value="{{$get_ad->price}}">
        ...
        <div class="row mb-3">
            ...
            <div class="col-8">
                <textarea name="description" class="form-control" id="" cols="30" rows="10">{{$get_ad->description}}</textarea>
            </div>
        </div>
        ...
        <div class="mb-3">
                    ...
                    <select name="city_id" size="11" class="form-select" id="city_id">
                        @if($cities->count())
                            @foreach($cities as $city)
                                <option value="{{$city->id}}" {{($get_ad->city_id === $city->id) ? 'selected' : ''}}>{{$city->title}}</option>
                            @endforeach
                        @else
            ...
            <div class="row mb-3">
                <h5 id="location_header">Обязательно укажите примерное расположение на карте если хотите поменять</h5>
            </div>
            <div class="row">
                <input type="text" name="coordinates" id="coords" value="{{$get_ad->coordinates}}" hidden>
                <input type="text" name="location" class="form-control" id="location" value="{{$get_ad->location}}" required readonly>
        ...
        <div class="row mb-3">
            ...
            <div class="col">
                <input type="radio" class="btn-check" name="animals" value="1" id="animals_true" autocomplete="off" {{($get_ad->animals === 1) ? 'checked' : ''}}>
                <label class="btn btn-outline-orange" for="animals_true" >Да</label>

                <input type="radio" class="btn-check" name="animals" value="0" id="animals_false" autocomplete="off" {{($get_ad->animals === 0) ? 'checked' : ''}}>
        ...
        <div class="row mb-3">
            ...
            <div class="col">
                @foreach($ad_gender_types as $type)
                    <input type="radio" class="btn-check" name="ad_gender_type_id" value="{{$type->id}}" id="ad_gender_type_{{$type->id}}" autocomplete="off" {{($get_ad->ad_gender_type_id == $type->id) ? 'checked' : ''}}>
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-6">
                <input type="text" class="form-control" name="contact_name" value="{{$get_ad->contact_name}}" required>
            </div>
        </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-6">
                <div class="d-flex align-items-center">
                    <input x-data="{ mask: '+79999999999' }" x-mask="+79999999999" name="phone_number" class="form-control form-orange ms-2" value="{{$get_ad->phone_number}}" required>
                </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-6">
                <div class="d-flex align-items-center">
                    <input type="email" name="contact_email" class="form-control" value="{{$get_ad->contact_email}}" required>
                </div>
    ...
    <form action="{{route('user.get_ad.destroy',['get_ad'=>$get_ad])}}" method="post">
        @csrf
        @method('DELETE')
